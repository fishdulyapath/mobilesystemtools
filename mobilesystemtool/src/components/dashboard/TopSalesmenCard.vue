<script setup>
import { ref } from 'vue'
import { getDashboardTopSalesmen } from '@/services/salesService'
...
async function load() {
  loading.value = true
  try {
    rows.value = await getDashboardTopSalesmen()
  } catch {
    rows.value = []
  } finally {
    loading.value = false
  ...
  <Card ref="containerRef">
    <template #title>
      <div class="card-title-row">
        ...
        <span>พนักงานขายดีเด่นประจำเดือน</span>
      </div>
    </template>
    <template #content>
      ...
      <ol v-else class="rank-list">
        <li v-for="(row, i) in rows" :key="row.sale_code" class="rank-item">
          <span class="rank-no" :class="`rank-${i + 1}`">{{ i + 1 }}</span>
          <div class="rank-detail">
            <span class="rank-name">{{ row.emp_name || row.sale_code }}</span>
            <span class="rank-sub">{{ row.sale_code }} · {{ row.total_docs }} ใบ</span>
          </div>
